...
// 移动pdfjs 至 versionPath
gulp.task('move:pdfjs',['concat:config'], function(){
  return gulp.src(`${distPath}/*.worker.js`)
...
gulp.task('default', ['clean'], function () {
  ...
  // 开始打包编译
  gulp.start(['build', 'create:versionCatalog', 'replace:cdnUrl', 'replace:version', 'concat:config', 'move:pdfjs'], function () {
    // 清除, 编译 / 处理项目中产生的文件
    del([`${distPath}/static`, `${distPath}/*.worker.js`, `${versionPath}/static/config`])
  })
});
